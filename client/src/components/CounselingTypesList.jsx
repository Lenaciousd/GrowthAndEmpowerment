...
const CounselingTypesList = () => {
 ...
 const counselingType = types.map((type, index) => {
   return <div key={index} className="coachingType" > <h2>{type.name}</h2> <p>{type.description}</p> <img src={type.img} /></div>
 })

  return (
    <div>
    <h2>What Area of your life would you like to focus on?</h2>
    {counselingType}
    </div>
  )
}
